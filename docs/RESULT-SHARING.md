...
順位は名前・タイムの照合や先頭50件からの推定ではなく、保存済みのクリアID／30分記録IDで取得します。通常は時間の昇順、同タイムでは登録IDの昇順です。30分は最終資産の降順、同額では登録IDの昇順。現金＋強化費の総資産ルールだけを対象にします。旧現金記録は保存のみ残し、順位照会・一覧・平均・掲示板バッジ・共有順位から除外します。旧セーブは進行を保持して総資産へ一方向に移行し、再計算した結果はランキング対象外の個人記録になります。
