...
初回の通常モードは「目標→音の試聴・確認→通知のおすすめ」の3ページです。通知案内も `intro` モーダル内に置き、案内中はゲームを進めません。許可要求は「通知をオンにする」のクリック時だけ実行し、許可後の開始・スキップで初回説明を完了します。拒否・未対応でも開始可能で、通知は後からヘッダーのベルで変更できます。既存の `INTRO_KEY` を維持するため、説明済みプレイヤーへの再表示はありません。通知ONで抽選確率や背景進行の設定は変わりません。30分モードは背景通知の案内を省きます。

ゲームの `.app` はCSSで文字選択を無効にし、WORKや切替の連打による意図しない選択を防ぎます。入力欄・セレクト・編集可能テキスト・コピー用URL・クリア記念画像は例外です。スクロール、フォーカス、画像の長押し保存を保つため、選択防止のイベントキャンセルや全体への `touch-action: none` は使いません。
