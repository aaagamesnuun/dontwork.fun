...
入替案内は標準で行いますが、初回当たり補助は標準OFFです。対象を初めて賭けるスピンは、独立したLAB設定 `secondBetAssist` がONの場合だけ有効な当たり出目を選びます。この設定はルール変更として比較用ランになります。受理済みの別ギャンブルの結果は変更せず、完了状態も結果公開まで隠します。途中のWORKによる公開ピーク到達を取りこぼさず、旧セーブで既に対象を解放済みの場合は新たな補助を与えません。設定項目のない既存セーブはOFFを補完し、進行・IDを維持します。30分モードは対象外です。
