...
入替案内は標準で行いますが、初回当たり補助は標準OFFです。対象を初めて賭けるスピンは、独立したLAB設定 `secondBetAssist` がONの場合だけ有効な当たり出目を選びます。この設定はルール変更として比較用ランになります。受理済みの別ポジションの結果は変更せず、完了状態も結果公開まで隠します。途中のWORKによる公開ピーク到達を取りこぼさず、旧セーブで既に対象を解放済みの場合は新たな補助を与えません。設定項目のない既存セーブはOFFを補完し、進行・IDを維持します。30分モードは対象外です。
...
- **ポジション:** 受け付け済みスピンの結果と予約資金は保持し、編成変更は次回へ反映します。公開済みジャックポットを終わらせる編成変更には確認状態`positionRequest`があります。
...
[trialUnlock.ts](../src/trialUnlock.ts)とAppが通常クリアから解放を記憶します。LABにも先行解放があります。通常プレイのセーブ・クリアカードは30分チャレンジに置き換えられません。
...
| `trial.anchor`と`Date.now()` | 開始・再開後の30分締切。AUTOが空編成や資金不足でも、開始中のチャレンジ時間は進む。一時停止はanchorを外す。 |
...
[backgroundPlay.ts](../src/backgroundPlay.ts)の背景進行は標準OFFで、LABから有効にします。Web Locks、効果音ON・音量あり、ジャックポット通知ON・通知許可が必要です。上限は1回の離席につき1時間・12,000スピンで、残り周期・演出待ち・30分の締切も計算に含めます。
...
最初のジャックポットで`backgroundJackpot`として停止し、結果を保存して復帰を待ちます。30分の時計もその時点で止めます。復帰時に前景で再開し、離席中の過去の音や派手な演出をまとめて再生しません。背景進行を使わない非表示遷移では、受け付け済み結果を反映し、30分を一時停止して保存します。これはサーバーで常時実行するジョブではなく、ブラウザの実行・通知制約を受ける処理です。
