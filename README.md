...
自由にフォークして、自分の dontwork.fun を作ってください。作ったら [Xの @realnuun](https://x.com/realnuun) に教えてもらえるとうれしいです。

Fork freely under the MIT license. If you make your own version, show [@realnuun on X](https://x.com/realnuun).
...
ランキングは日本時間の日次・週次（月曜始まり）・全体を切り替えられ、対象の全記録の平均を表示します。ヘッダーの掲示板では投稿・返信ができ、保存済みのランキング記録と登録名が一致すると名前の横に全体順位が付きます。集計方法と運用は [ランキングと掲示板](docs/community.md) を参照してください。

- ポジションとアップグレードを組み合わせる通常プレイ、時間制チャレンジ、LABの比較ルール。
- 日本語・英語、モバイル・デスクトップ、PWAに対応。
- ブラウザ内の自動保存とセーブの書き出し・読み込み。
- コードで生成する効果音と3種類のBGM。BGMは標準OFF。ヘッダーの調整アイコン「見た目・サウンド」で、8種類のUI・サウンドパック・BGMを選べます。起動ごとにUIをランダムにするチェックも用意しています。
- React / TypeScript / Vite。オンライン機能には任意でCloudflare Workers / D1を使用します。
- ハンバーガーメニューの「AIに遊ばせる」からCodex / Claude Code用の接続URLを発行。AIがAPI・MCPでプレイし、人間は観戦できます。詳細は[AIモード](docs/AI-MODE.md)。
