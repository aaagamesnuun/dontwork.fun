...
export function useAiSpectatorSound(snapshot: AiSnapshot | null, watching: boolean) {
  const [enabled, setEnabled] = useState(false), [pack, setPack] = useState(savedPack);
  const [played, setPlayed] = useState<{ cue: Cue; version: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const visibility = () => { cursor.reset(); stopSounds(); setPlayed(null); };
  ...
  useEffect(() => {
    ...
    if (!snapshot) { cursor.reset(); return; }
    const next = cursor.next(snapshot, audible);
    if (!next) return;
    if (next.kind === 'result') sound(next.cue, settings.current, next.power, next.accent);
    else uiSound(next.cue, settings.current);
    setPlayed({ cue: next.cue, version: snapshot.version });
  }, [snapshot, enabled, watching, cursor]);
  const toggle = () => {
    const next = !enabled;
    cursor.sync(snapshot);
  ...
  return <section className="ai-sound-panel" aria-label={t('AI観戦のサウンド')} data-last-cue={audio.enabled ? audio.played?.cue : undefined}>
    <div className="ai-sound-controls">
      <button type="button" className="ai-sound-toggle" aria-pressed={audio.enabled} aria-label={t(audio.enabled ? '観戦の音をOFFにする' : '観戦の音をONにする')} onClick={audio.toggle}>
        ...
        {audio.enabled && <span key={audio.played?.version ?? 'ready'} className={`ai-audio-led ${audio.played ? 'is-playing' : ''}`} aria-hidden="true"/>}
      </button>
