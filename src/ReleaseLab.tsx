...
export function ReleaseLab({ s, change, onWorkMode }: {
 ...
 <label className="setting-row"><span>{_t("AUTOボタンをなくす")}<small>{_t("通常モード専用。準備ができると自動で回ります。30分モードは砂時計で開始・一時停止します。")}</small></span><input type="checkbox" disabled={!!s.trial} checked={s.settings.autoAlwaysOn} onChange={e => set({autoAlwaysOn:e.target.checked})}/></label>
 ...
 <label className="setting-row"><span>{_t("画面の構成")}</span><select value={s.settings.workspaceMode} onChange={e => set({ workspaceMode: e.target.value as Settings['workspaceMode'] })}><option value="desk">{_t("1画面 · 標準")}</option><option value="tabs">{_t("旧タブ式 · チャート／ギャンブル／強化")}</option></select></label>
 ...
 <label className="setting-row"><span>{_t("最新の資産変化を総資産のすぐ右に表示")}</span><input type="checkbox" checked={s.settings.balanceChangeInline} onChange={e => set({ balanceChangeInline:e.target.checked })}/></label>
 ...
 <label className="setting-row"><span>{_t("コインフリップを試す")}<small>{_t("ONにすると最高資産が$10Kを超えた後、切り替えにコインフリップが追加されます。")}</small></span><input type="checkbox" checked={s.settings.coinFlip} onChange={e => set({ coinFlip:e.target.checked })}/></label>
