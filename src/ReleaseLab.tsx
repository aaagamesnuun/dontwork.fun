...
export function ReleaseLab({ s, change, onWorkMode }: {
 ...
 <label className="setting-row"><span>{_t("出目の表示")}</span><select value={s.settings.rollDisplay} onChange={e => set({ rollDisplay: e.target.value as Settings["rollDisplay"] })}><option value="dice">{_t("100面サイコロ · 標準")}</option><option value="number">{_t("数字だけ · 以前の表示")}</option></select></label>
