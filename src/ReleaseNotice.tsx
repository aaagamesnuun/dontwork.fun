...
export function ReleaseNotice({ check }: {
    ...
    return null; return <div className="version-notice" role="dialog" aria-modal="true" aria-label={_t("新しいバージョン")}><section><h2>{_t("新しいdontwork.funがあります。")}</h2><p>{_t("ホーム画面に追加する前に、最新版へ。続きはこの版で合言葉を発行して引き継げます。")}</p><a className="primary" href={check.latest.url}>{_t("最新版 v{0}へ →", check.latest.version)}</a><button className="secondary intro-start" onClick={check.dismiss}>{_t("この版を開く・合言葉を作る")}</button></section></div>; }
export function VersionLinks() { return <div><p>{_t("別のURLへ移るときは、先に「セーブ・合言葉」で続きを保存してください。")}</p><div className="version-links"><a href="https://dontwork.fun/">{_t("本番 · dontwork.fun →")}</a>{['2-3', '2-2', '2-1', '2-0', '1-11-0', '1-10-0', '1-9-0'].map(v => <a key={v} href={`https://bebullish-v${v}.realnuun.chatgpt.site/`}>v{v.replaceAll('-', '.')} →</a>)}</div></div>; }
